...
export default {
  ...
  computed: {
    ...
    withdrawControlSuggestions(){ if(!this.withdrawControlNo) return this.uniqueManualControlNos; const s=this.withdrawControlNo.toLowerCase(); return this.uniqueManualControlNos.filter(c=>c.toLowerCase().includes(s)); },
    withdrawInchargeSuggestions(){ if(!this.withdrawIncharge) return this.uniqueManualIncharge; const s=this.withdrawIncharge.toLowerCase(); return this.uniqueManualIncharge.filter(i=>i.toLowerCase().includes(s)); },
    withdrawLocationSuggestions(){ if(!this.withdrawProjectLocation) return this.uniqueManualLocations; const s=this.withdrawProjectLocation.toLowerCase(); return this.uniqueManualLocations.filter(l=>l.toLowerCase().includes(s)); },
  ...
  methods: {
    ...
    pickWithdrawControl(c){ this.withdrawControlNo=c; this.showWithdrawControlSug=false; },
    hideWithdrawControlSug(){ setTimeout(()=>{ this.showWithdrawControlSug=false; },150); },
    pickWithdrawIncharge(i){ this.withdrawIncharge=i; this.showWithdrawInchargeSug=false; },
    hideWithdrawInchargeSug(){ setTimeout(()=>{ this.showWithdrawInchargeSug=false; },150); },
    pickWithdrawLocation(l){ this.withdrawProjectLocation=l; this.showWithdrawLocationSug=false; },
    hideWithdrawLocationSug(){ setTimeout(()=>{ this.showWithdrawLocationSug=false; },150); },
